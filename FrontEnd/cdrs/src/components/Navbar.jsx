...
const Navbar = () => {
  ...
  return (
    <nav className="shadow-md py-5" style={{ color: "black" }}>
      <div className="container flex items-center justify-between flex-wrap">
        ...
        <div
          className={`${
            !isOpen ? "w-full flex flex-col mt-4" : "hidden"
        ...
        >
          {Links.map((link) => (
            <a
              href={link.ref}
              key={link.text}
              className="underline-offset-4 hover:underline duration-200 mb-4 lg:ml-7 lg:mb-0"
            >
              {link.text}
            </a>
          ))}
